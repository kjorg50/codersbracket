function (game, team1, team2) {
  ...
  if (round < 4) {
    ...
    // If it is the Elite Eight, make some #1 seeds lose.
    // I added this because I found that very often all four #1 seeds advance to the final four, 
    //  which I don't think is very probable.
    if (round == 2) {

      var oneLoses = Math.random();
      if(oneLoses <= 0.5){
        if( team1.seed == 1){
          team2.winsGame();
        } else if (team2.seed == 1){
    ...
    else { // not the elite eight

      var chanceOfUpset = Math.random();
      var rand1 = Math.random();
      var rand2 = Math.random();

      // I'll give a 40% chance of upset in the later rounds, where upset is determined by
      // official rank of a team, not seeding
      if(chanceOfUpset <= 0.40){
        if(rand1*team1.official_rank < rand2*team2.official_rank){
          team1.winsGame();
        } else {
          team2.winsGame();
        }
